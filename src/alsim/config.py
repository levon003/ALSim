...
class BinaryTextClassifierConfig:
    ...
    def get_configs(self, config_value_map):
        ...
        if len(config_key_list) == 0:  # special case: empty config_value_map should just produce this config itself
        ...
        while not finished_processing:
            ...
            config.update_from_dict({
                config_key: value_list[ind] for config_key, value_list, ind in zip(config_key_list, value_lists, inds)
            })
            configs.append(config)
            # update the inds
            i = len(inds) - 1
...
class Rq1and2Config(BinaryTextClassifierConfig):
    ...
    def get_configs(self):
        ...
        for config in all_configs:
            if config.metric_strategy == 'random_only_cv' and config.n_reserved_random == 0:
                continue
            if config.sample_strategy == 'all_random' and config.metric_strategy == 'random_only_cv':
                continue
