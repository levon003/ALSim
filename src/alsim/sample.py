...
def get_batch_sample(config, batch_size, curr_batch, n_batches, selected_training_docs, training_pool_docs, prev_batch_metrics, existing_classifier):
    """
    ...
    prev_n_train = prev_batch_metrics['n_train'] if 'n_train' in prev_batch_metrics else 0
    prev_n_train_pos = prev_batch_metrics['n_train_pos'] if 'n_train_pos' in prev_batch_metrics else 0
    ...
    if should_filter:
        if search_type == 'oracle':
            ...
            pos_inds_to_sample = np.random.choice(pos_inds, size=pos_to_sample_count, replace=False)
            
            neg_inds = np.nonzero(y == 0)[0]
            neg_inds_to_sample = np.random.choice(neg_inds, size=neg_to_sample_count, replace=False)
            ...
            assert len(pool_docs) == batch_size, f"Oracle retrieved more than the batch size ({len(pool_docs)} > {batch_size})."
        elif search_type == 'unigram' or search_type == 'bigram' or search_type == 'human':
            ...
            available_document_ids = set([doc['document_id'] for doc in training_pool_docs])
            ...
            while not identified_valid_search and i < len(search_query_options):
                ...
                pool_docs = [doc for doc in training_pool_docs
            ...
            if i == len(search_query_options) and not identified_valid_search:
                logger.warning(f"Exausted all search_query_options without finding a match. Bailing on search. {search_query_options}")
                should_filter = False
            if config.search_verbose:
                logger.info(f"Search query: '{search_query}'; {len(available_and_searched)} available docs, {len(pool_docs)} pool docs. ({len(search_query_options)} queries available, rejected {i-1} queries.)")
        else:
            raise ValueError(f"Unknown search type {search_type}.")
    ...
    if n_reserved_random > 0 and name != 'random':
        ...
        pre_selected_inds = list(np.random.choice(
            range(len(training_pool_docs)), 
            size=2, 
            replace=False))
        n_to_sample -= 2
        assert n_to_sample > 0

    if existing_classifier is None and sample_type in ['uncertainty', 'most_likely_positive']:
    ...
    if n_to_sample == len(pool_docs):
        ...
        selected_inds = list(np.random.choice(range(len(pool_docs)), 
                                         size=n_to_sample, 
                                         replace=False))
        ...
        assert len(pool_docs) == len(training_pool_docs), f"{len(pool_docs)}, {len(training_pool_docs)}"
        ...
        if 'is_kmeans_sample_eligible' not in pool_docs[0]:
            ...
            kmeans_pool_docs = [doc for doc in pool_docs 
                if doc['is_kmeans_sample_eligible']]
        ...
        if should_run_kmeans:
            ...
            kmeans_pool_docs = [doc for doc in pool_docs if doc['is_kmeans_sample_eligible']]
            # done with kmeans, eligible docs placed in kmeans_pool_docs
        
        # sample randomly from the kmeans_pool_docs
        kmeans_selected_inds = list(np.random.choice(range(len(kmeans_pool_docs)), 
                                         size=min(n_to_sample, len(kmeans_pool_docs)),
                                         replace=False))
        # convert inds in kmeans subset to full pool
        selected_inds = [pool_docs.index(kmeans_pool_docs[i]) for i in kmeans_selected_inds]
        ...
        scores = [
            document_id_score_dict[doc['document_id']] if doc['document_id'] in document_id_score_dict else 0.0 
        ...
        selected_inds = alsim.lcbal.run_lcbal(config, curr_batch, n_to_sample, pool_docs)
    else:
    ...
    if pre_selected_inds is not None:
        ...
        pre_selected_inds_tup = [(selected_ind, 'random') for selected_ind in pre_selected_inds]
        selected_inds_tup = pre_selected_inds_tup + [(selected_ind, sample_type) for selected_ind in selected_inds]
    else:
        selected_inds_tup = [(selected_ind, sample_type) for selected_ind in selected_inds]
