...
def log_summary(result):
    ...
    for i, cm in enumerate(result['combined_metrics_list']):
        logger.info(f"{i:>3}: {cm['true_metrics_f1_mean'] if 'true_metrics_f1_mean' in cm else -1:.4f} {cm['estimated_metrics_f1_mean'] if 'estimated_metrics_f1_mean' in cm else -1:.4f}")
    logger.info(f"F1 true (combined, mean and std): {result['combined_metrics_list'][-1]['true_metrics_f1_mean']:.4f} ({result['combined_metrics_list'][-1]['true_metrics_f1_std']:.4f})")
    logger.info(f"F1 estimated (combined, mean and std): {result['combined_metrics_list'][-1]['estimated_metrics_f1_mean']:.4f} ({result['combined_metrics_list'][-1]['estimated_metrics_f1_std']:.4f})")
...
def generate_and_run_batches(working_dir, args, override=False, n_processes=1):
    ...
    with mp.Pool(processes=n_processes) as pool:
        ...
        if len(existing_files) > 0:
            logger.info(f"Found {len(existing_files)} files (from a previous run?) that are not reflected in the configs generated for this run. Listing:")
            for filename in existing_files:
                logger.info(filename)
        for i, result in enumerate(pool_results):
            elapsed_time = result.get()
            logger.debug(f"Joined config {i} / {len(pool_results)} after {str(elapsed_time)}.")
...
def run_batch(config, output_filepath):
    ...
    logger.info(f"{os.getpid()}: Loaded {len(training_docs)} training and {len(testing_docs)} testing docs.")
    ...
    if config.search_type != 'oracle':
        ...
        if config.search_verbose:
            logger.info(f"{os.getpid()}: Loaded search data. ({config.search_data[config.outcome].keys()})")
    ...
    result = alsim.sim.train_and_evaluate_model_config(config, training_docs, testing_docs)
    #logger.info(f"Finished evaluation of config {config_hash} after {datetime.now() - s}.")
    
    # save results
    with open(output_filepath, 'wb') as outfile:
        pickle.dump(result, outfile)
    logger.info(f"Saved result from config {config_hash} to '{output_filepath}' after {datetime.now() - s}.")
...
def generate_and_run_max_data_batches(working_dir):
    ...
    training_docs, testing_docs = alsim.data.load_training_and_testing_docs()
    logger.info(f"Loaded {len(training_docs)} training and {len(testing_docs)} testing docs.")
    ...
    with open(output_filepath, 'w') as outfile:
        for config in tqdm(configs, desc='Processing max data configs'):
            ...
            # vectorize X
            X_train = alsim.classification.vectorize_docs(config.vector_source, training_docs)
            X_test = alsim.classification.vectorize_docs(config.vector_source, testing_docs)
            ...
            if config.use_bbsc and pos_pct > config.undersample_to_proportion and pos_pct < (1 - config.undersample_to_proportion):
            ...
            for i in range(config.n_runs):
                ...
                clf, unlabeled_valid_pct_pos = alsim.classification.train_model(X_train, y_train, X_train, config)
            ...
            outfile.write(f"{config.outcome}\t{config.use_bbsc}\t{config.undersample_to_proportion}\t{accuracy}\t{f1}\t{precision}\t{recall}\t{roc_auc}\t{ap}\t{true_pos_pct}\t{pred_pos_pct}\n")
            outfile.flush()
            logger.info(f"Trained max data '{config.outcome}' (BBSC={config.use_bbsc},u2p={config.undersample_to_proportion}) in {datetime.now() - s}. Acc={accuracy:.4f}; F1={f1:.4f}")
...
def main():
    ...
    parser.add_argument('--max-data', dest='max_data', required=False, action='store_true', default=False)
    ...
    parser.add_argument('--override', dest='should_override', required=False, action='store_true', default=False)
    parser.add_argument('--print-summary', dest='should_print_summary', required=False, action='store_true', default=False)
    ...
    if args.max_data:
        ...
        generate_and_run_batches(working_dir, args, override=args.should_override, n_processes=int(args.n_processes))
