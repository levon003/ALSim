...
def downsample_majority_class(majority_class, target_proportion, X, y):
    ...
    n_majority_class_to_keep = int(np.ceil((minority_count / target_proportion) - minority_count))
    ...
    majority_inds_to_keep = np.random.choice(majority_class_inds, size=n_majority_class_to_keep, replace=False)
...
def train_model(X, y, X_valid, config):
    ...
    unlabeled_valid_pct_pos = np.sum(y_unlabeled_valid_pred) / len(y_unlabeled_valid_pred)
    ...
    if config.use_bbsc:
        ...
        bbsc_clf = sklearn.linear_model.LogisticRegression(solver='liblinear', penalty='l2', class_weight=class_weights)
        bbsc_clf.fit(X, y)
