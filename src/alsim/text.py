...
def add_tokens(docs):
    ...
    for text_dict, tokenized_text in zip(docs, spacy_tokenizer.pipe(texts, batch_size=len(docs))):
...
def load_texts():
    fulltext_docs_filepath = os.path.join(alsim.paths.DERIVED_DATA_DIR, 'amazon_reviews_us', 'fulltext_docs.pkl')
