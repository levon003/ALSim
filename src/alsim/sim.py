...
def train_and_evaluate_model(
    curr_batch, config, selected_training_docs, validation_docs, testing_docs
):
    ...
    clf, unlabeled_valid_pct_pos = alsim.classification.train_model(
        X_train, y_train, X_unlabeled_validation, config
    )
    new_classifier = clf
    unlabeled_valid_pct_pos_list = [
        unlabeled_valid_pct_pos,
    ]
    ...
    if (
        config.sample_strategy == "lcbal"
        and config.use_lcbal_classifier
        and "lcbal_params" in config.__dict__
    ):
        clf = config.lcbal_params.clf
    elif (
        config.sample_strategy == "lcbal"
        and config.lcbal_use_rejection_sampling
        and "lcbal_params" in config.__dict__
    ):
        ...
        p_adjusted = lcbal_p_train / np.max(
            lcbal_p_train
        )  # TODO use a value other than the max? consider the approach used by Alejandro Correa Bahnsen http://albahnsen.github.io/CostSensitiveClassification/Sampling.html
        ...
        if config.lcbal_verbose:
            logger.debug(
                f"LCB-AL Rejection Sampling: {np.sum(to_include)} / {len(p_adjusted)} included in training data (n_pos = {np.sum(lcbal_y_train)} / {len(lcbal_y_train)})"
            )

        # train the classifier
        if n_pos >= 1 and n_pos < len(lcbal_y_train):
            clf, _ = alsim.classification.train_model(
                lcbal_X_train, lcbal_y_train, X_unlabeled_validation, config
            )
    ...
    if metric_strategy == "random_only_cv":
        randomly_sampled_docs = np.array(
            [doc["sample_type"] == "random" for doc in selected_training_docs]
        )
        if np.sum(randomly_sampled_docs) <= 1:
            ...
            if random_pos_count == 0 or random_pos_count == len(y_full):
                ...
                y_random_pred_proba = np.zeros(
                    len(randomly_sampled_doc_inds), dtype=float
                )
                for i, test_ind in enumerate(randomly_sampled_doc_inds):
                    ...
                    loo_clf, unlabeled_valid_pct_pos = alsim.classification.train_model(
                        X_train, y_train, X_unlabeled_validation, config
                    )
                    unlabeled_valid_pct_pos_list.append(unlabeled_valid_pct_pos)
                    y_pred = loo_clf.predict(np.reshape(X_test, (1, -1)))
                    y_pred_proba = loo_clf.predict_proba(np.reshape(X_test, (1, -1)))[
                        :, 1
                    ]
                    y_random_pred[i] = y_pred
                    y_random_pred_proba[i] = y_pred_proba
                estimated_metrics = alsim.metrics.compute_metrics(
                    y_random, y_random_pred, y_random_pred_proba
                )
    if metric_strategy == "cv":
        ...
        estimated_metrics = alsim.metrics.compute_metrics(
            y_full, y_full_pred, y_full_pred_proba
        )
    ...
    if metric_strategy == "bootstrap_merge":
        ...
        while len(train_estimated_metrics_list) < B:
            ...
            if (
                train_pos_count == 0
                or train_pos_count == len(y_train)
                or test_pos_count == 0
                or test_pos_count == len(y_test)
            ):
                # no or all positives in either the train or test fold
                # therefore, skip this bootstrap sample
                continue
            bootstrap_clf, unlabeled_valid_pct_pos = train_model(
                X_train, y_train, X_unlabeled_validation, config
            )
            unlabeled_valid_pct_pos_list.append(unlabeled_valid_pct_pos)
    ...
    if len(unlabeled_valid_pct_pos_list) > 0:
        ...
        ci = alsim.utils.compute_ci(
            len(unlabeled_valid_pct_pos_list), np.std(unlabeled_valid_pct_pos_list)
        )
        estimated_metrics["validation_pred_pos_pct_mean"] = mean_validation_pred_pos_pct
        estimated_metrics["validation_pred_pos_pct_upper"] = min(
            mean_validation_pred_pos_pct + ci, 0.99999
        )
        estimated_metrics["validation_pred_pos_pct_lower"] = max(
            mean_validation_pred_pos_pct - ci, 0.00001
        )
    ...
    if metric_strategy == "bootstrap":
        ...
        model_metrics[
            "bootstrap_loo_estimated_metrics"
        ] = bootstrap_loo_estimated_metrics
        bootstrap_loo_difference_metrics = {}
        for key in bootstrap_loo_estimated_metrics.keys():
            if key in true_metrics:
                bootstrap_loo_difference_metrics[key] = (
                    bootstrap_loo_estimated_metrics[key] - true_metrics[key]
                )
        model_metrics[
            "bootstrap_loo_difference_metrics"
        ] = bootstrap_loo_difference_metrics
...
def train_and_evaluate_model_config(config, training_docs, testing_docs):
    ...
    for run_ind in tqdm(range(config.n_runs), disable=not config.should_tqdm_runs):
        ...
        assert (
            len(training_pool_docs) >= 1000
        ), "Something weird is happening with the availability of training data; check inputs."
        ...
        for batch in range(n_batches):
            ...
            prev_batch_metrics = (
                batch_metrics_list[-1] if len(batch_metrics_list) > 0 else {}
            )
            selected_inds = alsim.sample.get_batch_sample(
                config,
                config.batch_size,
                batch,
                config.n_batches,
            ...
            for selected_ind_tup in sorted(
                selected_inds, reverse=True, key=lambda tup: tup[0]
            ):
            ...
            batch_metrics["total_time_seconds"] = (
    ...
    model_dict["combined_metrics_list"] = alsim.metrics.combine_run_metrics(
        run_metrics_list
    )

    model_dict["overall_elapsed_time_seconds"] = (
